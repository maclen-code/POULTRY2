...
class DashAcctAdapter internal constructor()
    : RecyclerView.Adapter<DashAcctAdapter.ViewHolder>() {

    private var list = emptyList<Data.SovDashAcct>() // Cached copy of words
    private var filteredList = emptyList<Data.SovDashAcct>()

    var onItemClick: ((Data.SovDashAcct)->Unit) ?= null
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        with (holder) {
            ...
            Table.createHeader(current.storeName,
                listOf("VOLUME","AMOUNT","LAST YEAR", "GROWTH",
                    "LAST MONTH","GROWTH"),binding.table1,true)

            table1(binding.table1,current.sovAcct)

            if (current.listAcctArSummary.isNotEmpty()) {
                Table.createHeader(
                    "", listOf(
                ...
                table2( binding.table2, current.listAcctArSummary)
            }
    ...
    private fun table2( table: TableLayout, list:List<Data.AcctArSummary>){
        val context=table.context
        val textColor = context.resolveColorAttr(android.R.attr.textColorSecondary)
        list.forEach { item ->
    ...
    internal fun setData(data: List<Data.SovDashAcct>) {
        this.list = data
        ...
        return object : Filter() {
            override fun performFiltering(charSequence: CharSequence): FilterResults {
                ...
                if (charString.isEmpty()) {
                    ...
                    val filteredList: MutableList<Data.SovDashAcct> = ArrayList()
                    for (row in list) {

                        // name match condition. this might differ depending on your requirement
                        // here we are looking for name or phone number match
                        if (row.storeName.lowercase(Locale.ROOT)
                                .contains(charString.lowercase(Locale.ROOT))
                        ) {
                            filteredList.add(row)
                        }
                    }
                    this@DashAcctAdapter.filteredList = filteredList
                }
            ...
            override fun publishResults(
                ...
                filteredList = filterResults.values as List<Data.SovDashAcct>
                notifyDataSetChanged()
            }
        }
    }
