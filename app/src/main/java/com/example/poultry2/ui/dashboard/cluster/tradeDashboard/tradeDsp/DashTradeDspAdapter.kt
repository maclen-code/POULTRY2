...
class DashTradeDspAdapter internal constructor()
    : RecyclerView.Adapter<DashTradeDspAdapter.ViewHolder>() {

    private var data = emptyList<Data.SovDashTradeDsp>() // Cached copy of words

    var onItemClick: ((Data.SovDashTradeDsp)->Unit) ?= null
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        with (holder) {
            ...
            val headers1=listOf("BUNIT","VOLUME","AMOUNT","","ORDERED","UNIVERSE","VARIANCE","","%",
                "LAST YEAR","GROWTH","LAST MONTH","GROWTH")
            Table.createHeader(
                Table.headerTitle(current.dsp,headers1[0]),headers1,binding.table1,true
            )

            table1(binding.table1, current.listSovDspBunit)

            val headers2= listOf("CHANNEL","VOLUME","AMOUNT","", "ORDERED", "UNIVERSE", "VARIANCE","",
                "%","LAST YEAR","GROWTH","LAST MONTH","GROWTH")
            Table.createHeader(
                Table.headerTitle(current.dsp,headers2[0]),headers2, binding.table2,true
            )

            table2(binding.table2, current.listSovDspChannel)
            if (current.listSovDspBunit.size > 1)
                table2Subtotal(binding.table2,current.listSovDspChannel)
    ...
    private fun table2( table: TableLayout, list:List<Data.SovDspChannel>){
        val context=table.context
        list.sortedByDescending { it.totalNet }.forEach {item ->
    ...
    private fun table2Subtotal( table: TableLayout, list:List<Data.SovDspChannel>){
        val context=table.context
    ...
    internal fun setData(data: List<Data.SovDashTradeDsp>) {
        this.data = data
