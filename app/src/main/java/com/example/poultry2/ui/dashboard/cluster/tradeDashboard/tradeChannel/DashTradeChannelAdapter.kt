...
class DashTradeChannelAdapter internal constructor()
    : RecyclerView.Adapter<DashTradeChannelAdapter.ViewHolder>() {

    private var data = emptyList<Data.SovDashTradeChannel>() // Cached copy of words

    var onItemClick: ((Data.SovDashTradeChannel)->Unit) ?= null
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        with (holder) {
            ...
            val headers1=listOf("DSP","VOLUME","AMOUNT","","ORDERED","UNIVERSE","VARIANCE","","%",
                "LAST YEAR","GROWTH","LAST MONTH","GROWTH")
            Table.createHeader(
                Table.headerTitle(current.channel,headers1[0]),headers1,binding.table1,true
            )

            table1(binding.table1, current.listSovChannelDsp)
            if (current.listSovChannelCategory.size > 1)
                table1Subtotal(binding.table1,current.listSovChannelDsp)
    ...
    internal fun setData(data: List<Data.SovDashTradeChannel>) {
        this.data = data
